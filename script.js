...
(() => {
  ...
  /* -------------------------------------------------------
     14. Inquiry forms → info@salahlawfirm.com
     ------------------------------------------------------- */
  const INQUIRY_EMAIL = 'info@salahlawfirm.com';
  ...
  async function submitInquiryForm(form) {
    ...
    if (honey?.value) return { ok: false, spam: true };
    ...
    if (json.success === false) throw new Error('submit_rejected');
    return { ok: true };
  }

  function wireInquiryForm(form, { successEl, errorEl }) {
    if (!form) return;

    const submitBtn = form.querySelector('button[type="submit"]');
    const btnLabel = submitBtn?.querySelector('span');
    const defaultLabel = btnLabel?.textContent || 'Submit';
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        const result = await submitInquiryForm(form);
        if (!result.ok) {
        ...
        showError(`Something went wrong sending your message. Please call ${'404-868-5562'} or email ${INQUIRY_EMAIL} directly.`);
        submitBtn?.removeAttribute('disabled');
        if (btnLabel) btnLabel.textContent = defaultLabel;
      }
    });
  ...
  wireInquiryForm($('#heroForm'), {
    successEl: $('#heroFormSuccess'),
    errorEl: $('#heroFormError'),
  });

  wireInquiryForm($('#contactForm'), {
    successEl: $('#contactSuccess'),
    errorEl: $('#contactFormError'),
  });
